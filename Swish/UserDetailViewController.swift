...
class UserDetailViewController: UITableViewController, UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func updateViews() {
        ...
        var dash = "|"
        if webLink == "" || twitterLink == "" {
            dash = ""
        }
        usernameLabel.text = "\(user.userName) | \(user.userUserName)"
        userBioTextView.text = "\(user.bio) \n \(webLink) \(dash) \(twitterLink) "
...
//    }
    ...
    override func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        if indexPath.row == 0 {
            tableView.rowHeight = self.view.frame.height * (1 / 4)
